...
public class VideoLibrarySubtitleController extends ABasicController {
    ...
    public ApiMessageDto<ResponseListDto<List<VideoLibrarySubtitleDto>>> adminList(@RequestParam Long videoLibraryId, Pageable pageable) {
        videoLibraryRepository.findById(videoLibraryId)
                .orElseThrow(() -> new NotFoundException("[Video Library] Not found", ErrorCode.VIDEO_LIBRARY_ERROR_NOT_FOUND));

        pageable = PageRequest.of(pageable.getPageNumber(), pageable.getPageSize(), Sort.by("createdDate").descending());
        Page<VideoLibrarySubtitle> page = videoLibrarySubtitleRepository.findByVideoLibrary_Id(videoLibraryId, pageable);
    ...
    public ApiMessageDto<ResponseListDto<List<VideoLibrarySubtitleDto>>> list(@RequestParam Long videoLibraryId) {
        videoLibraryRepository.findByIdAndStatus(videoLibraryId, BaseConstant.STATUS_ACTIVE)
                .orElseThrow(() -> new NotFoundException("[Video Library] Not found", ErrorCode.VIDEO_LIBRARY_ERROR_NOT_FOUND));

        String key = redisService.buildKey("video-library-subtitle", "list", videoLibraryId.toString());
    ...
    private void evictPublicSubtitleListCache(Long videoLibraryId) {
        redisService.delete(redisService.buildKey("video-library-subtitle", "list", videoLibraryId.toString()));
    }
}
